...
const StateChangeComponent = ({ airCall }) => {
  ...
  const stateChangeCall = async () => {
    try {
      ...
      const callIds = airCall.filter(call => call.is_archived === (airCallState === "Archive"));
      console.log("Filtered call IDs:", callIds.map(call => call.id));
      await Promise.all(callIds.map(call => archiveCall(call.id, airCallState)));
  ...
  const archiveCall = async (callId, currentCallState) => {
    try {
      ...
      const bodyContent = currentCallState === "Archive" ? { is_archived: false } : { is_archived: true };
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to archive call ${callId}: ${response.statusText} - ${errorText}`);
      }
  ...
  return (
    <div className='call-container'>
      <div className='call-container-left' onClick={() => stateChangeCall()}>
        ...
        {airCallState === "Archive" ? <p>Activity all calls</p> : <p>Archive all calls</p>}
      </div>
    </div>
  );
}
